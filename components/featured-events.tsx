...
const statusConfig = {
    available: { label: "Disponible", variant: "default" as const, className: "bg-olympic-green text-white" },
    limited: { label: "Dernières places", variant: "secondary" as const, className: "bg-olympic-yellow text-black" },
    soldout: { label: "Épuisé", variant: "destructive" as const, className: "bg-olympic-red text-white" },
}
                        ...
                        <Card key={event.id} className="group overflow-hidden hover:shadow-lg transition-shadow border-border/50">
                            <CardHeader className="pb-3">
                                <div className="flex items-start justify-between">
                                    ...
                                    <Badge className={statusConfig[event.status as keyof typeof statusConfig].className}>
                                        {statusConfig[event.status as keyof typeof statusConfig].label}
                                    </Badge>
                                </div>
                                <div className="space-y-1 pt-2">
                                    <p className="text-sm font-medium text-primary">{event.sport}</p>
